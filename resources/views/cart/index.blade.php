...
<div class="space-y-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <a href="{{ route('products.index') }}" class="btn-modern-secondary text-xs py-2.5 px-4 w-full sm:w-auto text-center">
    ...
    @if(empty($cart))
        <!-- Empty Cart State -->
        <div class="bg-white rounded-3xl p-8 sm:p-12 text-center border border-slate-100 shadow-sm max-w-xl mx-auto space-y-4">
            <div class="w-16 sm:w-20 h-16 sm:h-20 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center mx-auto text-2xl sm:text-3xl">
                <i class="fa-solid fa-cart-flatbed"></i>
            </div>
            <h3 class="text-xl sm:text-2xl font-bold text-slate-800">Your Cart is Empty</h3>
            <p class="text-slate-500 text-xs sm:text-sm">Looks like you haven't added any products to your shopping cart yet.</p>
            <div class="pt-2">
                <a href="{{ route('products.index') }}" class="btn-modern-primary py-3 px-8 text-xs sm:text-sm inline-block">
                    Browse Catalog
                </a>
            </div>
        </div>
    @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-white rounded-3xl p-4 sm:p-6 border border-slate-100 shadow-sm space-y-4">
                    <!-- Desktop Header Row -->
                    ...
                    <div class="divide-y divide-slate-100">
                        @foreach($cart as $id => $item)
                            <div class="py-4 flex flex-col sm:grid sm:grid-cols-12 gap-4 items-center">
                                ...
                                <!-- Mobile Footer Row: Price, Stepper, Subtotal -->
                                <div class="w-full sm:col-span-6 flex items-center justify-between sm:contents pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                                    <!-- Unit Price -->
                                    <div class="sm:col-span-2 text-left sm:text-center text-xs sm:text-sm font-semibold text-slate-700">
                                        <span class="sm:hidden text-slate-400 block text-[10px]">Price:</span>
                                        ${{ number_format($item['price'], 2) }}
                                    </div>

                                    <!-- Symmetrical Quantity Stepper -->
                                    <div class="sm:col-span-2 flex justify-center">
                                        <div class="inline-flex items-center rounded-xl border border-slate-200 overflow-hidden bg-slate-50 shadow-xs h-8 sm:h-9">
                                            <form action="{{ route('cart.update', ['id' => $id]) }}" method="POST" class="h-full flex items-center">
                                                @csrf
                                                <input type="hidden" name="quantity" value="{{ max(1, $item['quantity'] - 1) }}" />
                                                <button type="submit" aria-label="Decrease quantity" class="w-8 sm:w-9 h-full flex items-center justify-center text-slate-600 hover:bg-slate-200 active:bg-slate-300 font-extrabold text-xs sm:text-sm transition-colors cursor-pointer">
                                                    -
                                                </button>
                                            </form>

                                            <span class="w-8 sm:w-10 text-center font-extrabold text-xs text-slate-800 border-x border-slate-200 bg-white h-full flex items-center justify-center select-none">
                                                {{ $item['quantity'] }}
                                            </span>
                                            ...
                                            <form action="{{ route('cart.update', ['id' => $id]) }}" method="POST" class="h-full flex items-center">
                                                ...
                                                <button type="submit" aria-label="Increase quantity" class="w-8 sm:w-9 h-full flex items-center justify-center text-slate-600 hover:bg-slate-200 active:bg-slate-300 font-extrabold text-xs sm:text-sm transition-colors cursor-pointer">
                                                    +
                                                </button>
                                            </form>
                                        </div>
                                    ...
                                    <!-- Subtotal Price -->
                                    <div class="sm:col-span-2 text-right font-extrabold text-blue-600 text-sm sm:text-base">
                                        <span class="sm:hidden text-slate-400 block text-[10px] font-normal">Subtotal:</span>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-6">
                    ...
                    <a href="{{ route('checkout.index') }}" class="btn-modern-primary w-full py-3.5 text-center text-sm font-bold uppercase tracking-wider shadow-lg shadow-blue-500/25 inline-block">
