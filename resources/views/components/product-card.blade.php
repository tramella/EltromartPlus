...
<div class="product-card-modern product-card-gsap group">
    ...
    <form action="{{ route('wishlist.toggle', ['id' => $id]) }}" method="POST" class="absolute top-3 right-3 z-30">
        @csrf
        <button type="submit" aria-label="Add to Wishlist" class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/90 shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-white transition-all transform hover:scale-110 cursor-pointer">
    ...
    <div class="product-image-wrapper">
        <a href="{{ $detailUrl }}" class="w-full h-full flex items-center justify-center">
            <img src="{{ asset('images/' . $imageName) }}" alt="{{ $name }}" loading="lazy" width="220" height="220" class="object-contain group-hover:scale-105 transition-transform duration-300" onError="this.onerror=null;this.src='{{ asset('images/sp1.jpg') }}';" />
        </a>
    </div>
    ...
    <!-- Unified Single Action Buttons Bar (Clean layout for ALL screens) -->
